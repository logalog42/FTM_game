local name = "starter"				-- mod name of vehicle

local definition = {
	terrain_type = 2,						-- 0 = air, 1 = land, 2 = liquid, 3 = land + liquid
	description = "This is were is all starts",			-- name as seen in inventory
	collisionbox = { -0.5, -0.5, -0.5, 0.5, 0.5, 0.5 },		-- back, bottom, starboard, front, top, port
	...
	can_fly = true,					-- if enabled, the specified vehicle will be able to fly around
	...
	inventory_image = "ship_core.png",		-- image to use in inventory
	wield_image = "ship_core.png",			-- image to use in hand
	wield_scale = {x=1, y=1, z=1},			--
	visual = "cube",				-- what type of object (mesh, cube, etc...)
	mesh = "",				-- mesh model to use
	textures = {"ship_core.png"},			-- mesh texture(s)
...
ship.register_vehicle("ship:"..name, definition)
